...
<div class="container">
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body">
            ...
            <form id="courseForm" action="{{ route('courses.update', $course->id) }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="short_name" class="form-control @error('short_name') is-invalid @enderror" 
                               value="{{ old('short_name', $course->short_name) }}" maxlength="20">
                        @error('short_name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="class_type" class="form-select @error('class_type') is-invalid @enderror" required>
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type }}" 
                                    {{ old('class_type', $course->class_type) == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                              rows="3" maxlength="500">{{ old('description', $course->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                {{-- ✅ Multiple Section Inputs with Class Name --}}
                <div class="mb-4">
                    <label class="form-label">Sections <span class="text-danger">*</span></label>
                    <div id="section-container">
                        @php
                            // Get sections from old input or from course model
                            $sections = [];
                            if (old('sections')) {
                            ...
                            if (empty($sections)) $sections = [['name' => '', 'class_name' => $course->name]];
                        @endphp
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <input type="text" name="sections[{{ $index }}][class_name]" 
                                               ...
                                               value="{{ $section['class_name'] ?? $course->name }}" 
    ...
    courseNameInput.addEventListener('input', function() {
        ...
        if (className) {
            ...
            classNameInputs.forEach(input => {
                if (!input.value.trim() || input.value === '{{ $course->name }}') {
